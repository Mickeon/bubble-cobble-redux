...
global.draw_custom_screen_overlay = (gui_graphics, delta_tracker) => {
	if ((!Client.player || Client.player.mainHandItem.isEmpty() || !Client.player.mainHandItem.block)) {
		elapsed_time = 0.0
		return
	}
	if (!$ShapeMap.contains(Client.player.mainHandItem)) {
		return
	}

	// Client.statusMessage = Client.isKeyBindDown("key.clutternomore.change_block_shape")
	// if (Client.isKeyBindDown("key.clutternomore.change_block_shape")) {
	// 	return
	// }
