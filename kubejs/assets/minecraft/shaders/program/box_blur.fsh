...
void main() {
    ...
    // Blur on the bottom-left specifically (aimed at Tips).
    float actualRadius = round(Radius * RadiusMultiplier);
    ...
    for (float a = -actualRadius + 0.5; a <= actualRadius; a += 2.0) {
        blurred += texture(DiffuseSampler, texCoord + sampleStep * a * strength);
    }
    blurred += texture(DiffuseSampler, texCoord + sampleStep * strength) / 2.0;
    fragColor = blurred / (actualRadius + 0.5);
}
